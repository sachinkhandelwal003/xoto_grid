...
import {
  ...
  FiFileText, FiMail, FiCopy, FiEye, FiImage,
} from "react-icons/fi";
...
// ─────────────────────────────────────────────
// PRESENTATION MODAL (same as GridAgentLeadDetail)
// ─────────────────────────────────────────────
...
const [showLeadModal, setShowLeadModal] = useState(false);
  ...
  const handleGenerateOffer = async (actionType = 'download') => {
    ...
    try {
      ...
      const currentTranslations  = { EN: translations.EN, [activeLang]: translations[activeLang] || translations.EN };
  ...
  const getStatusTag = () => {
    ...
    if (property.propertySubType === "off_plan") return <Tag color="purple" style={{ borderRadius: 8, fontWeight: 600 }}>🏗️ Off-Plan Project</Tag>;
