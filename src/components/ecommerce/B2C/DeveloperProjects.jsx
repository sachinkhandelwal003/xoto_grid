    ...
    try {
      ...
      const mapped = list.map((p, i) => {
        ...
        return {
          ...
          units:             p.floorPlans?.length > 0 ? `${p.floorPlans[0]?.areaFrom || 0} - ${p.floorPlans[0]?.areaTo || 0} sqft` : null,
          ...
          // ── Interest / Wishlist count from backend ──────────────
          interestCount: p.favourites?.length || p.wishlistCount || p.interestedCount || 0,
